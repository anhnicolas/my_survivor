...
const Statistics = () => {
    ...
    const updateCoachCharts = (coachStats) => {

        fetchCustomersByCoach(coachStats._id);
        const customerCountBySign = calculateCustomerCountBySign(selectedCoachCustomers);
        console.log('Customer count by sign:', selectedCoachCustomers);
        setPieChartData(customerCountBySign);
    
        const coachingEvolution = coachStats.coachingsByWeek || {};
        const sortedWeeks = Object.keys(coachingEvolution).sort();
        const lineChartData = [
            {
                name: t("coachingEvolution"),
                data: sortedWeeks.map(week => coachingEvolution[week] || 0)
            }
        ];
    ...
    const calculateCustomerCountBySign = (customers) => {
        ...
        customers.forEach(customer => {
            const translatedSign = t(customer.astrologicalSign);
